...
export const UserEnvironmentVariable = {
  ...LambdaEnvironmentVariableName,
  USER_POOL: "GRID_WOLF_USER_POOL",
};
...
export class UserStack extends StepintoBaseStack {
  ...
  createEventHandler(props: UserStackProps, userPool: UserPool) {
    const eventBusArn = StringParameter.valueForStringParameter(
      this,
      `/${props.env.prefix}${parameterNames.EVENT_BUS_ARN}`
    );
    ...
    const workingPolicy = new PolicyDocument({
      statements: [
        new PolicyStatement({
          ...
          resources: [
            `arn:aws:dynamodb:${props.env.region}:${props.env.account}:table/${props.env.prefix}-${props.dataTableName}`,
          ],
        }),
        new PolicyStatement({
          effect: Effect.ALLOW,
    ...
    const lambda = new LambdaFunction(this, this.generateId("event-handler"), {
      ...
      role,
      environment: {
        [UserEnvironmentVariable.EVENT_BUS_ARN]: eventBusArn,
        [UserEnvironmentVariable.DATA_TABLE_NAME]: `${props.env.prefix}-${props.dataTableName}`,
        [UserEnvironmentVariable.USER_POOL]: userPool.userPoolId,
      },
    });
